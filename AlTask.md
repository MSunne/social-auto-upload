...
## 逻辑调整（工程级）
整体思路：omnibull_frontend主要负责账号管理，任务查看，客户在omnibull_frontend实现账号的增删改查，然后立即同步到OmniDrive，客户在omniDrive上创建任务，然后同步列表下来，时间点到了后OmniBull自动去云端拉取任务结果到本地分发。
- omnibull_frontend菜单栏只保留仪表盘，账号管理，任务中心，其余删除。
- OmniBull账号管理，客户点击账号管理，可以增加账号，正常拉起三方和验证，账号添加成功后，将信息保存在本地，只是同步账号添加成功的信息给OmniDrive，如果没有正常同步到OmniDrive可以手动点击同步按钮，同步账号到OmniDrive，OmniDrive也可以点击同步获得账号列表。
- OmniDrive中不再支持账号验证，只同步状态，OmniBull中每4小时自动使用对应账号进入对应平台一次，如果不能成功进入，则自动记录为cookie失效，并立即同步到云端，如果当前有发布任务，则中断发布任务，等待用户的cookie生效后，重新发布，要弹窗征求用户的同意，用户同意后才发布，用户不同意，则按照时间安排，等待后续任务即可。
- OmniDrive现在云端点击添加账号的按钮，点击后，出现详细的账号添加指引（指引客户在OmniBull设备上，访问localhost:5409添加账号，然后点击同步）
- 现在OmniDrive的三方账号管理需要大改（抖音，视频号，小红书，快手）均使用这个，其他平台我额外使用API形式统一管理发布。
- OmniBull现在支持账号验证，点击验证账号使用对应cookie拉起对应的平台，查看能否进入，不能进入则记录为失效。
- OmniBull现在作为一个全新要面对用户的交互前端，现在美感设计要重新设计