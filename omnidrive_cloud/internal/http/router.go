...
func NewRouter(app *appstate.App) stdhttp.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(private chi.Router) {
			...
			private.Route("/skills", func(skills chi.Router) {
				...
				skills.Get("/defaults", skillHandler.EditorDefaults)
			...
			private.Route("/digital-human", func(digitalHuman chi.Router) {
				digitalHuman.Get("/models", digitalHumanHandler.Models)
				digitalHuman.Get("/billing-preview", digitalHumanHandler.BillingPreview)
			...
			private.Route("/billing", func(billing chi.Router) {
				...
				billing.Get("/commissions/{commissionId}/releases", billingHandler.ListCommissionReleases)
				billing.Get("/packages", billingHandler.ListPackages)
				billing.Get("/rules", billingHandler.ListPricingRules)
				billing.Get("/activities", billingHandler.ListActivities)
	...
	r.Route("/api/admin/v1", func(admin chi.Router) {
		...
		admin.Group(func(private chi.Router) {
			...
			private.With(authmiddleware.RequireAdminPermission("user.delete")).Delete("/users/{userId}", adminConsoleHandler.DeleteUser)
			...
			private.With(authmiddleware.RequireAdminPermission("device.update")).Post("/devices/{deviceId}/unbind", adminConsoleHandler.UnbindDevice)
			...
			private.With(authmiddleware.RequireAdminPermission("finance.read")).Get("/pricing/workflow-duration-rules", adminConsoleHandler.ListWorkflowDurationRules)
			...
			private.With(authmiddleware.RequireAdminPermission("finance.adjust")).Post("/pricing/workflow-duration-rules", adminFinanceHandler.CreateWorkflowDurationRule)
			private.With(authmiddleware.RequireAdminPermission("finance.adjust")).Patch("/pricing/workflow-duration-rules/{ruleId}", adminFinanceHandler.UpdateWorkflowDurationRule)
			...
			private.With(authmiddleware.RequireAdminPermission("finance.read")).Get("/billing-activities", adminConsoleHandler.ListBillingActivities)
			...
			private.With(authmiddleware.RequireAdminPermission("support_recharge.review")).Get("/support-recharges/lookup", adminConsoleHandler.LookupSupportRecharge)
			...
			private.With(authmiddleware.RequireAdminPermission("support_recharge.review")).Post("/support-recharges/{orderId}/invalidate", adminConsoleHandler.InvalidateSupportRecharge)
			...
			private.With(authmiddleware.RequireAdminPermission("distribution.settle")).Patch("/distribution/partners/{userId}", adminDistributionHandler.UpdatePartner)
			...
			private.With(authmiddleware.RequireAdminPermission("distribution.settle")).Patch("/distribution/relations/{relationId}", adminDistributionHandler.UpdateRelation)
			...
			private.With(authmiddleware.RequireAdminPermission("distribution.read")).Get("/distribution/commissions/{commissionId}/releases", adminDistributionHandler.ListCommissionReleases)
